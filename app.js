...
function init() {
  ...
  controls.addEventListener('start', () => {
    ...
    // When user starts rotating, remove the white background for better contrast
    const darkBG = new THREE.Color(0x0f1114);
    scene.background.copy(darkBG);
    if (scene.fog) scene.fog.color.copy(darkBG);
    renderer.setClearColor(scene.background);
  });
  ...
  btnEnter.addEventListener('click',       () => enterExperience(false));
  btnEnterSilent.addEventListener('click', () => enterExperience(true));
...
function triggerWickIgnition(idx) {
  ...
  // audioController.triggerIgnitionWhoosh(); // removed: thread fire sound
